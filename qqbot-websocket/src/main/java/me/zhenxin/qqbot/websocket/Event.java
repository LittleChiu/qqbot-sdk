    ...
    public void onDispatch(Payload payload) {
        ...
        switch (e) {
            ...
            case "MESSAGE_AUDIT_PASS":
                ...
                client.getEventHandler().onMessageAuditPass(messageAuditPassEvent);
                break;
            ...
            case "INTERACTION_CREATE":
                Interaction ic = JSON.toJavaObject((JSONObject) payload.getD(), Interaction.class);
                log.info("机器人在群聊 {} 中被允许主动推送消息, 操作者: {}", ic.getGroupOpenid(), ic.getGroupOpenid());
                InteractionCreateEvent interactionCreateEvent = new InteractionCreateEvent(this,ic);
                client.getEventHandler().onInteractionCreate(interactionCreateEvent);
                break;
            case "RESUMED":
